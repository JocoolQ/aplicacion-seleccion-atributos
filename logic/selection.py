...
#         matriz_entropia = similarity_matrix.copy()
...
class Selector:
    ...
    def aplicar_seleccion_numerica(self, df):
        n_result = {}
        n_entropies = []
        total_entropy = round(self.seleccionar_atributos_numericos(df), 3)
        n_result['Características'] = 'Entropías'
        n_result['General'] = total_entropy
        for i in df.columns:
            entropy = round(self.seleccionar_atributos_numericos(df.drop(i, axis = 1).copy()),3)
    ...
    def aplicar_seleccion_categorica(self, df, filename):
        ...
        c_entropies = []
        c_num_attrs = []
        total_entropy = round(self.seleccionar_atributos_categoricos(df), 3)
        c_result['Características'] = ['Entropías', 'Número de categorías']
        c_result['General'] = [self.seleccionar_atributos_categoricos(df), 0]
        for i in df.columns:
            info = []
            entropy = round(self.seleccionar_atributos_categoricos(df.drop(i, axis = 1).copy()), 3)
